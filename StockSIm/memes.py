...
st.markdown("""
    ...
    .stInfo { background-color: #002b36; border-left: 5px solid #268bd2; color: #93a1a1; }
    </style>
...
while True:
    with placeholder.container():
        ...
        for i, (name, data) in enumerate(st.session_state.market_data.items()):
            # Logic: Random drift + News Impact
        ...
        if 'user' not in st.session_state:
            ...
            st.sidebar.write(f"Held: `{owned}` shares")
            ...
            if b_col.button("BUY", use_container_width=True):
                ...
                else: st.sidebar.error("Funds Error")
            ...
            if s_col.button("SELL", use_container_width=True):
                ...
                else: st.sidebar.error("Stock Error")
        ...
        for name, profile in users.items():
            current_port = sum(profile["portfolio"][t] * st.session_state.market_data[t]["price"] for t in STARTING_CONFIG)
            rankings.append({"Trader": name, "Net Worth": profile["balance"] + current_port})
        
        df_rank = pd.DataFrame(rankings).sort_values("Net Worth", ascending=False)
        st.sidebar.table(df_rank.head(5))

        # 9. BACKGROUND UPDATES
        if random.random() < 0.20:
            st.session_state.current_news = random.choice(NEWS_POOL)
        else:
            # Decay the impact so news doesn't pump stock forever
